...
### Test Coverage
- 289 tests passing
- 28 test files covering: parser, facts, schema, rules (catalog, set, matrix, schema_gate, multivdom, ordering, new_controls, password_policy, idle_timeout, dns_default_only, firmware_outdated, ssh_weak_ciphers, snmp_no_acl, cert_expiring, fgfm_default_override), versioning, authority, baseline, CLI, report, build_corpus, readme

### Builtin Rules (28 total)
...
1. Add new rules (FGT-IFACE-NO-VLAN-SECURITY, FGT-DHCP-SNOOP) — target 30+ rules
2. Expand schema field extraction for security-critical tables beyond priority 17
3. Add model.py unit tests (untested module — HIGH priority per Track 7)
