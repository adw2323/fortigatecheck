...
- Parser flattening also applies to antivirus profile protocol sections: `config http` inside `edit "profile"` becomes a root-level `("http",)` table, losing the profile association
